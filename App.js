...
import {Image, View, Text, ActivityIndicator,ScrollView,TouchableHighlight, Dimensions, BackHandler} from 'react-native';
...
export default class ImV extends Component {
	constructor(props){
		super(props)
  ...
  render() {
	...
	return (
			<Image source = {{uri: this.state.pic.originalUrl}} style={{height:this.state.dim.height,width:this.state.dim.width}} >
  				<View key = {keys()} style = {{flex:1,flexDirection:'row',height:this.state.dim.height,width:this.state.dim.width}}>
					...
					<TouchableHighlight key = {keys()} onPress={()=>{return this.onPressImage2()}}>
						<View key = {keys()} style ={{flex:1, flexDirection:'column', justifyContent:'space-between', height:this.state.dim.height, width:3*this.state.dim.width/5}}>
